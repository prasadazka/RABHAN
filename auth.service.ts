...
async register(data: ContractorRegisterRequest): Promise<AuthTokens> {    const client = await this.pool.connect();        try {      await client.query("BEGIN");            const passwordValidation = PasswordUtils.validate(data.password);      if (!passwordValidation.valid) {        throw new Error(passwordValidation.errors.join(", "));      }            const passwordHash = await PasswordUtils.hash(data.password);      const normalizedPhone = data.phone ? ValidationUtils.normalizeSaudiPhone(data.phone) : null;      let phoneVerified = false;      if (normalizedPhone) {        const PhoneVerificationService = require("./phone-verification.service").PhoneVerificationService;        const phoneVerificationService = new PhoneVerificationService();        phoneVerified = await phoneVerificationService.isPhoneVerified(normalizedPhone);                if (!phoneVerified && config.env === "production") {          throw new Error("Phone verification required before registration");        }      }            // Step 1: Create user record in users table first      const createUserQuery = ;            const userResult = await client.query(createUserQuery, [        data.first_name,        data.last_name,        data.email.toLowerCase(),        passwordHash,        normalizedPhone,        AuthProvider.EMAIL,        data.national_id,        UserStatus.PENDING,        "BUSINESS", // Always set as BUSINESS for contractors        UserRole.CONTRACTOR,        false,        phoneVerified      ]);            const user = userResult.rows[0];            // Step 2: Create contractor record in contractors table with user_id reference      const createContractorQuery = ;            const contractorResult = await client.query(createContractorQuery, [        user.id, // Reference to the user record        data.company_name || "To be updated",        data.cr_number || null,        data.vat_number || null,        data.user_type === "BUSINESS" ? "llc" : "individual"      ]);            const contractor = contractorResult.rows[0];            const sessionId = uuidv4();      const { accessToken, refreshToken, expiresIn } = JWTUtils.generateTokenPair(        user.id, // Use user.id for token (not contractor.id)        user.email,        UserRole.CONTRACTOR,        sessionId      );            // Create session using user_sessions table      const createSessionQuery = ;            const expiresAt = new Date(Date.now() + JWTUtils.getExpiresInMs(config.jwt.refreshTokenExpiresIn));      await client.query(createSessionQuery, [sessionId, user.id, refreshToken, expiresAt]);            await client.query("COMMIT");            SAMALogger.logAuthEvent("CONTRACTOR_REGISTRATION", user.id, {        role: UserRole.CONTRACTOR,        provider: AuthProvider.EMAIL      });            // Cache user data (not contractor data)      await this.cacheUserData(user.id, user);            return {         accessToken,         refreshToken,         expiresIn,        user: {          id: user.id,          first_name: user.first_name,          last_name: user.last_name,          email: user.email,          role: UserRole.CONTRACTOR,          phone: user.phone,          national_id: user.national_id,          user_type: user.user_type,          status: user.status,          bnpl_eligible: false        }      };          } catch (error) {      await client.query("ROLLBACK");            if (error instanceof Error) {        if (error.message.includes("duplicate key value violates unique constraint")) {          if (error.message.includes("users_email_key")) {            throw new Error("Email already registered");          }          if (error.message.includes("users_national_id_key")) {            throw new Error("National ID already registered");          }        }      }            logger.error("Contractor registration failed:", error);      throw error;    } finally {      client.release();    }  }
    ...
    try {
      if (data.email && this.redis) {
        ...
        const isDevCredential = isDevelopment && !forceOTPInDev && (
          ...
          data.email && devCredentialEmails.includes(data.email)
        );
