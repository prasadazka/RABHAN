  async register(data: ContractorRegisterRequest): Promise<AuthTokens> {
    ...
    try {
      ...
      // Step 2: Create contractor record in contractors table with user_id reference
      const createContractorQuery = `
        INSERT INTO contractors (
          user_id, company_name, cr_number, vat_number, business_type
        )
        VALUES ($1, $2, $3, $4, $5)
        RETURNING *
      `;
      ...
      const contractorResult = await client.query(createContractorQuery, [
        user.id, // Reference to the user record
      ...
      const { accessToken, refreshToken, expiresIn } = JWTUtils.generateTokenPair(
        user.id, // Use user.id for token (not contractor.id)
        user.email,
        UserRole.CONTRACTOR,
        sessionId
      );
